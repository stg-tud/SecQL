...
# End time is reduced statically by 5s, since client only gets cold after for min 5s seconds nothing happened (real end might be even up to 5 seconds earlier)
measurementFinished = filter(eventData, grepl("^section\\.measurement-finished\\.enter$", event)) %>% group_by(node, execution) %>% summarize(timeFinished = median(time) - 4000) %>% ungroup
offsetNFinish = inner_join(timeOffsets, measurementFinished, by = c("node", "execution"))

# Apply time offsets and measurement end to the data
eventData = left_join(eventData, offsetNFinish, by = c("node", "execution")) %>% mutate(time = time - timeOffset) %>% arrange(time)
performanceData = left_join(performanceData, offsetNFinish, by = c("node", "execution")) %>% filter(time > timeOffset & time < timeFinished) %>% mutate(time = time - timeOffset) %>% arrange(time)
throughputData = left_join(throughputData, offsetNFinish, by = c("node", "execution")) %>% filter(time > timeOffset & time < timeFinished) %>% mutate(time = time - timeOffset) %>% arrange(time)
...
eventSpanData = inner_join(eventSpanEnters, eventSpanExits, by = c("node", "execution", "event", "timeOffset")) %>% mutate(duration = exitTime - enterTime)

# Init section events data
sectionSpanData = filter(eventSpanData, grepl("^section\\.", event))
```
...
durPlot(df = allBarrierPairs, start = "enterTime.x", end="enterTime.y", facet = TRUE, plot_type = "density", group = "execution", title = TRUE, title_density = "Section enter delay distribution")
```
